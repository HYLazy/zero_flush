...
static uint64_t FLAGS_vlog_size = 64 << 20;
...
class Benchmark {
 ...
 private:
  ...
  void Open() {
    ...
    Status s = DB::Open(options, FLAGS_db, FLAGS_db, &db_);
    if (!s.ok()) {
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (leveldb::Slice(argv[i]).starts_with("--benchmarks=")) {
      ...
      FLAGS_db = argv[i] + 5;
    } else if (sscanf(argv[i], "--vlog_size=%d%c", &n, &junk) == 1) {
      FLAGS_vlog_size = n;
    } else {
  ...
  leveldb::my_stats.var_key_value_size =
      18 + FLAGS_value_size + encoded_original_value_size;

  uint32_t vlog_size_tmp = FLAGS_vlog_size;
  ...
  leveldb::my_stats.max_vlog_size =
      max_num_log_item * (12 + leveldb::my_stats.var_key_value_size);

  leveldb::my_stats.clean_threshold = std::ceil((double)max_num_log_item * 0.25);
