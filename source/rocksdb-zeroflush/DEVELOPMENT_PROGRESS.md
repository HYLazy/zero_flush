...
**50GB 尺度（R19 终态验收，2026-08-23）**：upper_conflict 删除 + cache 256MB
使 50GB 前半段显著改善（34GB 前稳定 11-13MB/s、停写少——对比 R8 25GB/R17
21GB 停滞），但 34GB 后回落 L0 循环仍现（37GB 停滞、0.14MB/s）——upper_
conflict 删除只延后循环，批次粒度 vs 融合 ratio 的根源待攒批修复（M4.5b，
SV 时序修复后落地）。
